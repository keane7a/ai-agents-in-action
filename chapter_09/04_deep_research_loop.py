...
from _01_research_state_plan import SubTopic, ResearchPlan, ResearchState
from _03_iteration_output import ResearchIteration, SubTopicUpdate
...
def apply_plan_updates(plan: ResearchPlan, iteration: ResearchIteration) -> None:
    ...
    for new_st in iteration.new_sub_topics:
        if new_st.name not in existing:
            plan.sub_topics.append(
                SubTopic(name=new_st.name, status=new_st.status, notes=new_st.notes)
            )
...
async def run_research_loop(goal: str, max_iterations: int = 10) -> ResearchState:
    ...
    async with search_server:
        ...
        while state.should_continue:
            ...
            if iteration.goal_satisfied:
                ...
                print(f"Goal satisfied (confidence: {iteration.confidence:.0%})")
...
async def main():
    state = await run_research_loop(
        goal="What are the lastest advances in solid-state batteries and which companies are leading commercialisation?",
        max_iterations=5,
    )
    print(f"\n{'='*50}")
