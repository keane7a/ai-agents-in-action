...
class ResearchPlan(BaseModel):
    ...
    def progress_summary(self) -> str:
        ...
        complete = sum(1 for st in self.sub_topics if st.status == "complete")
        in_progress = sum(1 for st in self.sub_topics if st.status == "in_progress")
        return f"{complete}/{total} complete, {in_progress} in progress"
