...
async def run_collaboration_loop(goal: str, max_rounds: int = 5) -> CollaborationState:
    ...
    async with search_server:
        agents[0] = researcher_agent.clone(mcp_servers=[search_server])
        state = CollaborationState(goal=goal, max_rounds=max_rounds)
        ...
        while state.round_number < state.max_rounds and not state.has_consensus:
            ...
            print(f"\nRound {state.round_number + 1}, " f"Agent: {current_agent.name}")

            result = await Runner.run(current_agent, input=str(collab_input))
...
async def main():
    ...
    print(f"\n{'='*50}")
    print(f"Collaboration complete after " f"{state.round_number} rounds")
    ...
    for agent_name in [
        ...
        count = sum(1 for c in state.contributions if c.agent_name == agent_name)
