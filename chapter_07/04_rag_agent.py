...
# Update docstring for agent tool use instruction. 
@function_tool
def search_knowledge(query: str) -> dict: 
    """  Search the knowledge database for relevant facts. It compares keywords only.  
...
evaluation_agent = Agent(
    ...
    You will compare the answer against the expected answer key term to validate correctness. 
    """, 
...
for benchmark in _benchmarks: 
    ...
    evaluation = asyncio.run(
        Runner.run(evaluation_agent, input=str(eval_input))
    ).final_output
