...
while True:
    ...
    if not selected_document_index.isdigit() or not 0 <= int(
    ...
    fig = go.Figure([go.Bar(x=x_axis_labels, y=selected_document_similarities)])

    fig.update_layout(
        title=f"Cosine Similarities of '{documents[selected_document_index]}' with Others",
